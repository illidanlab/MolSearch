...
def zinc250k():
    """
    calculate estimated solubility for zinc250k molecules.
    return: zinc250k.csv with solubility column added 
    """
    esol_calculator = ESOLCalculator()
    df = pd.read_csv("zinc250k_property.csv")
    smiles_list = df['smile'].to_list()
    esol = []
    for i in range(len(smiles_list)):
        s = smiles_list[i]
        mol = Chem.MolFromSmiles(s)
        esol.append(esol_calculator.calc_esol(mol))
        if i%10000==0:
            print(i)
    df['esol'] = esol
    df.to_csv("zinc250k_esol.csv", index=False)


def calculate_niclosamide():
    esol_calculator = ESOLCalculator()
    s = 'C1=CC(=C(C=C1[N+](=O)[O-])Cl)NC(=O)C2=C(C=CC(=C2)Cl)O'
    mol = Chem.MolFromSmiles(s)
    esol = esol_calculator.calc_esol(mol)
    print('Niclosamide Solubility: {:.4f}'.format(esol))
    print(esol)


def zinc250k_46():
    df = pd.read_csv("zinc250k_esol.csv")
    df = df[df['esol'] > -5.01]
    df = df[df['esol'] < -4.99]
    print(df.shape)
    df.to_csv('zinc1k_esol.csv', index=False)


def delaney_corr():
    df = pd.read_csv("delaney.csv")
    arr1 = df['measured log(solubility:mol/L)'].to_numpy()
    arr2 = df['ESOL predicted log(solubility:mol/L)'].to_numpy()
    cor = pearsonr(arr1, arr2)
    print(cor)
